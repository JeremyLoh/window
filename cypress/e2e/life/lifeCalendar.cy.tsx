...
describe("life calendar", () => {
  ...
  function submitDateOfBirth(date: Date) {
    ...
    getDateOfBirthForm().find("[data-test='submit-date-of-birth']").click()
  }
  ...
  context("date of birth input", () => {
    ...
    function assertInvalidDateOfBirthInputCss() {
      getDateOfBirthInput().should(
        "have.css",
        "border-color",
        "rgb(236, 72, 153)"
      )
      getDateOfBirthInput().should("have.css", "color", "rgb(220, 38, 38)")
    }
    ...
    it("should show date of birth input", () => {
      ...
      const expectedMinYear: number = sub(currentDate, {
        years: MAX_YEARS,
      }).getFullYear()
      getDateOfBirthInput()
        .invoke("attr", "min")
      ...
      getDateOfBirthInput().invoke("attr", "max").should("eq", expectedMaxDate)
    })

    it("should allow date of birth input change", () => {
      getDateOfBirthInput().type("1999-12-31")
      getDateOfBirthInput().invoke("val").should("eq", "1999-12-31")
  ...
  context("generate life calendar based on date of birth", () => {
    ...
    it("should show life calendar grid of one past week", () => {
      ...
      const expectedEndDate: Date = add(oneWeekAgoDateOfBirth, {
        years: MAX_YEARS,
      })
      // Subtract 1 for present week
      const expectedFutureWeeks: number =
        differenceInCalendarISOWeeks(expectedEndDate, today) - 1
      submitDateOfBirth(oneWeekAgoDateOfBirth)
      getLifeCalendarContainer().find(".past").should("have.length", 1)
    ...
    it("should show life calendar grid of zero past weeks", () => {
      ...
      const expectedFutureWeeks: number =
        differenceInCalendarISOWeeks(expectedEndDate, today) - 1
      submitDateOfBirth(today)
      getLifeCalendarContainer().find(".past").should("have.length", 0)
      getLifeCalendarContainer().find(".present").should("have.length", 1)
      getLifeCalendarContainer()
        .find(".future")
        .should("have.length", expectedFutureWeeks)
    ...
    it("should show life calendar grid of one future week", () => {
      ...
      getLifeCalendarContainer().find(".present").should("have.length", 1)
      getLifeCalendarContainer().find(".future").should("have.length", 1)
    })
  ...
  context("desktop tests", () => {
    const sizes: (ViewportPreset | [number, number])[] = [
      "macbook-16",
      [1366, 768],
      [1600, 900],
      [1920, 1080],
    ...
    sizes.forEach((size) => {
      context(`${size} resolution`, () => {
        ...
        it(
          "should download desktop life calendar as png image when download button is" +
            " clicked",
          () => {
            const today = new Date()
            getLifeCalendarDownloadButton().should("not.exist")
            submitDateOfBirth(today)
            getLifeCalendarDownloadButton()
              .should("be.visible")
